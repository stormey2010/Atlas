...
def get_db_connection():
    return sqlite3.connect('memory/long-term.db')
...
def insert():
    ...
    insert_item(name, description, importance, keywords, userid=None)
...
def insert_item(name, description, importance, keywords, userid):
    with get_db_connection() as conn:
        db = conn.cursor()
        db.execute('''
        INSERT INTO items (name, description, importance, keywords, userid)
        VALUES (?, ?, ?, ?, ?)
        ''', (name, description, importance, keywords, userid))
        conn.commit()
...
def search_items(keywords, userid):
    with get_db_connection() as conn:
        db = conn.cursor()
        if not keywords:
            return []
        
        query = "SELECT * FROM items WHERE userid = ? AND ("
        conditions = []
        params = [userid]
